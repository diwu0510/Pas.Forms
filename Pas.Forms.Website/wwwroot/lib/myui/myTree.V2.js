(function ($) {
    ...
    $.fn.TreeView = function (settings) {
        ...
        function setNode(arr, nodeData, level, parent) {
            ...
            flatData[itemIdx] = nodeData;      // 把数据存入数组
            parentData[itemIdx] = parent;
            ...
            if (c && $.isArray(c) && c.length > 0) {
                ...
                if (_sets.isDisabledWhenHasChildren || !a) {
                    ...
                    arr.push(" class='enabled' ");
                }
                arr.push("><span class='blank' style='width: ", level * 18, "px'></span>");
                if (_sets.expandLevel && level < _sets.expandLevel) {
                ...
                arr.push(t);
                arr.push("</dt><dd");
                if (_sets.expandLevel != undefined && level >= _sets.expandLevel) {
                ...
                arr.push("<dt ", (a ? "class='enabled' " : " class='disabled' "), "data-id='", itemIdx, "' data-val='", v, "'>");
                arr.push("<span class='blank' style='width: ", level * 18, "px;'></span>");
                arr.push("<span>|-</span> ");
                if (settings.isMulti) {
                    arr.push("<input type='checkbox' id='cb", itemIdx, "' value='", itemIdx, "' data-val='", v, "'");
        ...
        function setItemClickListener() {
            ...
            me.on("click", "dt:not('.disabled')", function () {
                if (!_sets.isMulti) {
            ...
            }).on("dblclick", "dt:not('.disabled')", function () {
                if (!_sets.isMulti) {
                    var i = $(this).data("id");
                    var item = flatData[i];
        ...
        me.getByValue = function (val) {
            if (!_sets.isMulti) {
                var result = null;
                $.each(flatData, function (idx, item) {
